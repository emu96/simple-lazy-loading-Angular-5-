...
function addMessage(user, message, isSelf) {
  ...
  if (filename[0] === 'https:' && filename[1] === "" && filename[3] !== 'stm.patient') {
    var timestamp = new Date();
    if (isSelf) {
      var div = document.createElement('div');
      div.className = "message-block writer";
      ...
      var div = document.createElement('div');
      div.className = "message-block reciever";
      div.innerHTML = '<div class="message-box"><div class="file-container"><a href=' + message + ' target="_blank">' + message + '</a></div></div><p class="message-time">' + timestamp.getHours() + ':' + timestamp.getMinutes() + '</p>';
      messageList.appendChild(div);
      messageList.scrollTop = messageList.scrollHeight;
    }
...
function addFile(user, message, isSelf) {
  ...
  if (isSelf) {
    ...
    div.innerHTML = '<div class="message-box"><div class="file-container file-up" style="width: 11em; word-wrap:break-word;text-overflow: ellipsis; white-space: nowrap;  overflow: hidden;   padding: 5px";>' +
      '<a href=' + message + '><img src="assets/img/icons/download.svg"></a>' + filename[5] + '</div></div><p class="message-time">' + timestamp.getHours() + ':' + timestamp.getMinutes() + '</p>';
    messageList.appendChild(div);
    messageList.scrollTop = messageList.scrollHeight;
  }
  else {
